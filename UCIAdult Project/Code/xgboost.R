...
start.time <- Sys.time()
for(i in 20:nrow(xgb_param1)) {
  params <- list(
    eta = xgb_param1[i, "eta"],
    max_depth = xgb_param1[i, "max_depth"],
    subsample = xgb_param1[i, "subsample"],
    colsample_bytree = xgb_param1[i, "colsample_bytree"],
    gamma = xgb_param1[i, "gamma"],
    min_child_weight = xgb_param1[i, "min_child_weight"]
  )
  nrounds = xgb_param1[i, "nrounds"]
  res = xgb.cv(params,
               dtrain,
...
save(all.models, xgb_param2, dtrain, dvalidation, file="xgb_handtuning.rda")
